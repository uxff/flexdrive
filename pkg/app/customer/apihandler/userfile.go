...
func UserFileNewFolder(c *gin.Context) {
	...
	if req.DirName == "" {
		JsonErrMsg(c, ErrInvalidParam, "要创建的文件夹名称为空，请重新提交")
		return
	}
	...
	if strings.Contains(req.DirName, "/") {
		JsonErrMsg(c, ErrInvalidParam, "要创建的目录名称中不能有/等特殊字符")
		return
	}
	...
	if req.ParentDir != "/" && parentDirEnt == nil {
		JsonErrMsg(c, ErrInvalidParam, "选择的父目录("+req.ParentDir+")不存在")
		return
		//dao.MakeUserFilePath(userInfo.UserId, req.ParentDir)
	}
	...
	if existEnt != nil {
		JsonErrMsg(c, ErrInternal, "选择的目录("+req.ParentDir+req.DirName+")已经存在")
		return
		//dao.MakeUserFilePath(userInfo.UserId, req.ParentDir)
	}
	...
	if err != nil {
		JsonErrMsg(c, ErrInvalidParam, "创建目录("+req.ParentDir+")失败")
		return
	}
...
func UserFileRename(c *gin.Context) {
	...
	if existNameFile != nil {
		JsonErrMsg(c, ErrNameDuplicate, "提交的文件名已存在")
		return
	}
