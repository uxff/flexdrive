...
// 	if mid <= 0 {
// 		StdErrResponseJson(c, ErrInvalidParam)
// 		return
// 	}

// 	mgrEnt, _ := dao.GetManagerById(mid)
// 	if mgrEnt == nil {
// 		StdErrResponseJson(c, ErrMgrNotExist)
// 		return
// 	}

// 	if mgrEnt.Status == base.StatusDeleted {
// 		StdErrResponseJson(c, ErrMgrDisabled)
// 		return
// 	}
...
// ManagerAddForm - both for add and edit
func ManagerAddForm(c *gin.Context) {
	...
	if mid > 0 {
		...
		if req.Pwd == "" {
			JsonErrMsg(c, ErrInvalidParam, "密码不能为空")
...
func ManagerEnable(c *gin.Context) {
	...
	if enable == 1 {
		...
		if mgrEnt.IsSuperRole() {
			JsonErrMsg(c, ErrInternal, "不能停用超级管理的账号")
			return
		}
